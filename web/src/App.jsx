...
export default function App() {
  ...
  const [gateways, setGateways] = useState([
    { id: 0, name: 'GW_VIVO', srcip: '192.168.0.10', monitorip: '8.8.4.4', delay: 16.35, loss: 0, status: 'online' },
    { id: 1, name: 'WANGW', srcip: '192.168.100.10', monitorip: '8.8.8.8', delay: 0, loss: 100, status: 'down' }
  ]);
  ...
  const fetchGateways = async () => {
    ...
    try {
      ...
      console.log('Usando dados em cache do pfSense');
    } finally {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { 
        sender: 'bot', 
        text: '⚠️ Não foi possível conectar ao Core local. Tentando rota de contingência...', 
        time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) 
      }]);
    } finally {
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 flex-1 w-full">
        ...
        <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-red-950/60 to-slate-900 border border-red-900/80 flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-200 text-sm">🚨 INCIDENTE DETECTADO: Gateway WANGW com 100% de Perda de Pacotes</h3>
              <p className="text-xs text-red-300/80 mt-0.5">
                IP Monitor: 8.8.8.8 • Interface 192.168.100.10 • O link da operadora externa falhou. Tráfego contido no GW_VIVO.
              </p>
            </div>
          </div>
          <button 
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-white flex items-center gap-2">
                ...
                Status dos Gateways de Internet (pfSense)
              </h2>
              <span className="text-xs text-slate-400">Endpoint: /api/v2/status/gateways</span>
            </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs bg-slate-950/60 p-3 rounded-xl border border-slate-800/80 mb-3">
                      <div>
                        ...
                        <span className="font-mono text-slate-200 font-semibold">{gw.srcip}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block">IP de Teste (Monitor):</span>
                        <span className="font-mono text-slate-200 font-semibold">{gw.monitorip}</span>
                      </div>
                    </div>
            ...
            {/* SEÇÃO RÁPIDA DE FERRAMENTAS */}
            <div className="p-6 rounded-2xl bg-slate-900/40 border border-slate-800">
              <h3 className="text-sm font-semibold text-slate-300 mb-3">Ações Rápidas Disponíveis</h3>
              <div className="flex flex-wrap gap-3">
                ...
                >
                  💾 Auditar Snapshots no S3
                </button>
                <button 
                  onClick={() => { setActiveTab('chat'); setInputMsg('mostrar logs de auditoria'); }}
                  className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold border border-slate-700 transition"
                >
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400">Tokens e senhas protegidos com padrão AES-256-GCM. Apenas hashes cifrados residem no banco.</p>
              </div>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-800/70 text-slate-200">
                  <tr className="hover:bg-slate-800/30">
                    <td className="px-5 py-4 font-semibold text-white">pfSense Matriz</td>
                    <td className="px-5 py-4 text-xs font-mono text-sky-400">pfrest REST API v2</td>
                    <td className="px-5 py-4 text-xs font-mono text-slate-300">https://libra-vivo.awecloudsolution.com:8181</td>
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="px-5 py-4 font-semibold text-white">Proxmox Cluster Node 1</td>
                    <td className="px-5 py-4 text-xs font-mono text-sky-400">PVE REST API :8006</td>
                    <td className="px-5 py-4 text-xs font-mono text-slate-300">https://pve.awecloudsolution.com:8006</td>
                    <td className="px-5 py-4 text-xs font-mono text-slate-500">•••••••••••••••• (AES-256)</td>
                    <td className="px-5 py-4">
                      <span className="px-2 py-0.5 rounded-full text-xs bg-emerald-950 text-emerald-400 border border-emerald-800 font-medium">
                        Conectado
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="px-5 py-4 font-semibold text-white">Mikrotik CCR Borda</td>
                    <td className="px-5 py-4 text-xs font-mono text-sky-400">RouterOS API :8728</td>
                    <td className="px-5 py-4 text-xs font-mono text-slate-300">192.168.0.1:8728</td>
                    <td className="px-5 py-4 text-xs font-mono text-slate-500">•••••••••••••••• (AES-256)</td>
                    <td className="px-5 py-4">
                      <span className="px-2 py-0.5 rounded-full text-xs bg-emerald-950 text-emerald-400 border border-emerald-800 font-medium">
                        Conectado
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
                <span className="text-xs text-slate-400">Snapshots Válidos (Últimas 24h)</span>
                <p className="text-2xl font-bold text-emerald-400 mt-1">100%</p>
                <p className="text-[11px] text-slate-500 mt-0.5">Todos os nós sincronizados</p>
              </div>
              <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
                <span className="text-xs text-slate-400">Espaço Ocupado no S3</span>
                <p className="text-2xl font-bold text-white mt-1">428.6 GB</p>
                <p className="text-[11px] text-slate-500 mt-0.5">Bucket: nocagent</p>
              </div>
              <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
                <span className="text-xs text-slate-400">Última Auditoria Realizada</span>
                <p className="text-2xl font-bold text-sky-400 mt-1">04:00 AM</p>
                <p className="text-[11px] text-slate-500 mt-0.5">Rotina de integridade OK</p>
              </div>
            </div>
              ...
              <div className="space-y-2 text-xs">
                <div className="p-3 rounded-xl bg-slate-950/50 border border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <span className="font-semibold text-white">vzdump-qemu-101-2026_09_11.vma.zst (srv-database)</span>
                      <span className="block text-slate-400 text-[11px]">Proxmox VE • 24.8 GB • Storage S3</span>
                    </div>
                  </div>
                  <span className="text-emerald-400 font-medium">Íntegro</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-950/50 border border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                    <div>
                      <span className="font-semibold text-white">config-pfSense-matriz-20260911040000.xml</span>
                      <span className="block text-slate-400 text-[11px]">pfSense • 1.4 MB • Storage S3</span>
                    </div>
                  </div>
                  <span className="text-emerald-400 font-medium">Íntegro</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-950/50 border border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                    <div>
                      <span className="font-semibold text-white">backup-mikrotik-ccr-20260911.rsc</span>
                      <span className="block text-slate-400 text-[11px]">Mikrotik RouterOS • 480 KB • Storage S3</span>
                    </div>
                  </div>
                  <span className="text-emerald-400 font-medium">Íntegro</span>
                </div>
